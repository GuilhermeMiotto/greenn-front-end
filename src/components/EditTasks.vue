...
export default {
  ...
  methods: {
    updateTaskOnServer() {
  if (this.isDateValid) {
    const [day, month, year] = this.editedTask.due_date.split("/");
    const formattedDate = `${year}-${month}-${day}`;
    this.editedTask.due_date = formattedDate;

    axios
      .put(
        `http://127.0.0.1:8000/api/tasks/${this.editedTask.id}`,
        this.editedTask
      )
      .then((response) => {
    ...
    handleDateInput() {
      ...
      let isDateValid = regexDate.test(this.editedTask.due_date);
      ...
      if (isDateValid) {
        ...
        isDateValid = taskDate >= cutoffDate;
      }
